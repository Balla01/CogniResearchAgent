...
def google_search(query: str, num_results: int = 2, max_chars: int = 500) -> list:  # type: ignore[type-arg]
    ...
    search_engine_id = "d42ba021527284c1d"#os.getenv("GOOGLE_SEARCH_ENGINE_ID")
...
async def call_agent():
    # Define your topic
    topic = "What are the key features of the latest SpaceX Starship flight?"
    
    # Run the agent
    result = await search_agent.run(task=topic)
...
async def main():
    await Console(
        team.run_stream(task="Write a financial report on American airlines")
    )
